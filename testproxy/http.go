...
// HTTP creates an HTTP/HTTPS/CONNECT proxy service you can use to test proxy behavior.
func HTTP(t *testing.T, options ...HTTPOption) HTTPService {
...
// HTTPService is an HTTP/HTTPS/CONNECT proxy service for testing purposes.
...
// HTTPOptionForceHTTPTarget forces non-CONNECT (HTTP/HTTPS) requests to be sent to the specified target via an
// HTTP request regardless of the request. You should specify the target as hostname:ip.
func HTTPOptionForceHTTPTarget(target string) HTTPOption {
...
// HTTPOptionForceHTTPSTarget forces non-CONNECT (HTTP/HTTPS) requests to be sent to the specified target via an
// HTTPS request. If the backing server is using a custom CA, you should pass the caCert as the second parameter.
func HTTPOptionForceHTTPSTarget(target string, caCert []byte) HTTPOption {
...
// HTTPOptionForceCONNECTTarget forces CONNECT requests to be sent to the specified target, regardless
// of the request.
func HTTPOptionForceCONNECTTarget(target string) HTTPOption {
...
// HTTPOption is a function that changes the settings for the proxy server. The parameter is intentionally not
// exposed.
type HTTPOption func(options *httpProxyOptions) error
...
func (h *httpProxyService) start() error {
	...
	h.httpServer = &http.Server{
		...
		Handler:   h,
		TLSConfig: nil,
		ErrorLog:  testlog.NewGoTestLogger(h.t),
		BaseContext: func(_ net.Listener) context.Context {
			return testcontext.DefaultContext(h.t)
		},
...
func (h *httpProxyService) waitForService() error {
	...
	ctx, cancel := context.WithTimeout(testcontext.DefaultContext(h.t), httpProxyTimeoutUp)
	defer cancel()
	httpUp := false
	httpsUp := false
	var err error
